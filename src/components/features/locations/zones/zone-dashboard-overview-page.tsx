...
import { ZoneOverviewDashboardSkeleton } from '@/components/features/locations/zones/components/zone-overview-dashboard-skeleton'
import { DashboardZoneOverviewView } from '@/components/features/locations/zones/zone-overview-dashboard'
...
export function ZoneDashboardOverviewPageClient({ zoneId }: { zoneId: string }) {
  ...
  const [data, setData] = useState<ZoneOverviewDashboardData | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await dashboardApiClient.get<ApiResponse<ZoneOverviewDashboardData>>(
      ...
      setError('Could not load zone overview. Please try again.')
      setData(null)
    } finally {
      setIsLoading(false)
    }

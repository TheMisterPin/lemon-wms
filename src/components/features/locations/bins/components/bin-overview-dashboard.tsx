...
/** Quantity that matches the row's operational status (single column vs avail/res/blocked split). */
function pertinentQuantityForBinContent(row: BinContentRow): number {
  switch (row.status) {
  case 'AVAILABLE':
  ...
  case 'BLOCKED':
    return row.blocked
  case 'IN_TRANSIT':
    return row.available + row.reserved + row.blocked
  default: {
            ...
            {contents.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-left text-sm">
                  ...
                  <tbody>
                    {contents.map((row) => {
                      const qtyTone = stockStatusTone(row.status)
                      const qtyColor = warehouseOverviewToneStyles[qtyTone].icon
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="max-w-56 truncate py-2 pr-3" title={`${row.name} · Last op: ${row.lastOperationLabel}`}>
                            {row.name}
                          </td>
                          ...
                          <td className="py-2 pr-2 text-right text-sm font-medium tabular-nums">
                            <span style={{ color: qtyColor }}>
                              {pertinentQuantityForBinContent(row).toLocaleString()}
                            </span>{' '}
                            <span className="text-xs font-normal" style={{ color: 'var(--wh-text-muted)' }}>
                              {row.uom}
                            </span>
                          </td>
                          <td className="py-2 pr-3">
                            <WarehouseOverviewStatusPill tone={qtyTone}>

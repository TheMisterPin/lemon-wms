...
type UserActivity = { id: string; actionType: string; entityType: string | null; createdAt: string }
type ExecutionActivity = { id: string; activityType: string; orderType: string; createdAt: string }
type ActivitiesData = { userActivities: UserActivity[]; executionActivities: ExecutionActivity[] }
...
export function SalesActivitiesSheet({ open, onOpenChange, orderId }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    axios
      .get<ApiResponse<ActivitiesData>>(`/api/simulation/orders/sales/${orderId}/activities`)

...
/**
 * @generated-doc-link
 * @doc .docs/developer/refactors/components/component/dashboard/stock/inventory-health-page-client.md
 */

import { InventoryHealthDashboard } from '@/components/features/stock/components/inventory-health-dashboard'
import { Skeleton } from '@/components/ui/skeleton'
import { useInventoryHealthDashboard } from '@/hooks/dashboard/stock/use-inventory-health-dashboard'

export function InventoryHealthPageClient() {
  const { data, isLoading, error, refetch } = useInventoryHealthDashboard()
    ...
    return (
      <main className="min-h-screen p-4 xl:p-6" style={{ background: 'var(--wh-page-bg)' }}>
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <Skeleton className="h-28 rounded-2xl bg-wh-card-bg-soft" />
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-center p-6 xl:p-10">
          ...
          >
            <p className="text-sm" style={{ color: 'var(--wh-text-primary)' }}>
              {error ?? 'Could not load inventory health dashboard.'}
            </p>
            <button
              type="button"
  ...
  return (
    <main className="min-h-screen" style={{ background: 'var(--wh-page-bg)' }}>
      <InventoryHealthDashboard data={data} />
    </main>
  )
}

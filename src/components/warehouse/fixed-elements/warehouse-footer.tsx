...
import { useMoveItems } from '@/hooks/warehouse/use-move-items'
...
export default function WarehouseFooter() {
  ...
  async function handleUnloadSelection() {
    if (!destinationBinId) {
      setLocalError('Open a bin details page to unload trolley items.')

      return
    }
    if (selectedItems.length === 0) {
      setLocalError('Select at least one item in the trolley.')
  ...
  async function handleConfirmSingleQuantity() {
    ...
    if (!Number.isFinite(parsedQuantity) || parsedQuantity <= 0) {
      setLocalError('Enter a valid quantity greater than zero.')
      ...
      <Sheet open={isSheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="right" className="w-full max-w-md">
          <SheetHeader>
            ...
            <SheetDescription>
              Select one or more items and unload them into the current bin.
            </SheetDescription>
          </SheetHeader>
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              The selected item has more than one unit. Enter how many to unload.
            </DialogDescription>
          </DialogHeader>
          <NumericKeypad
